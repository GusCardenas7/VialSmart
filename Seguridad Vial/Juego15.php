...
<!-- MENU -->
    <?php 
       ...
       // Verifico si existe ya un registro con esos datos
        $sql = "SELECT * FROM juegos WHERE nombre='Prevencion y seguridad en caso de desastres' AND tipo='quiz'  AND lecciones_id = $id_lecciones AND lecciones_modulos_id = $id_modulos";
        $res = $con->query($sql);
...
function revisarpuntaje(){
    if(aciertos >= 6){
         ...
         if (xhr.readyState === 4) {
             if (xhr.status === 200) {
                ...
                var xhr2 = new XMLHttpRequest();
                        ...
                        if (xhr2.readyState === 4) {
                            if (xhr2.status === 200) {
                              ...
                              if(puntos === 100) {
                                ...
                                xhr3.send("nombre='Prevención ante emergencias'&id_usuario=" + <?php echo $id_usuario; ?> + "&puntos=" + puntos); 
                            }
                            } else {
                    ...
                    xhr2.send("nombre='Conocedor de las situaciones de emergencia'&id_usuario=" + <?php echo $id_usuario; ?>);
             } else {
       ...
       xhr.send("id_juego=" + <?php echo $id_juego; ?> + "&puntaje="+puntos + "&id_modulos=" + <?php echo $id_modulos; ?> + "&nombre_leccion='Identificacion de situaciones de riesgo'" + "&nombre_modulo='Seguridad vial y prevencion de delitos'" + "&id_usuario=" + <?php echo $id_usuario; ?>+ "&puntajeInicial=" + <?php echo $puntajeInicial; ?>);
