 ...
 <!-- MENU -->
    <?php 
       ...
       // Verifico si existe ya un registro con esos datos
        $sql = "SELECT * FROM juegos WHERE nombre='Comportamiento seguro en el automovil' AND tipo='adivinanza'  AND lecciones_id = $id_lecciones AND lecciones_modulos_id = $id_modulos";
        $res = $con->query($sql);
...
function cargarNuevaPalabra(){
    ...
    if(cantPalabrasJugadas>5){
        if(TotalAciertos>=3){
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4) {
                  if (xhr.status === 200) {
                    ...
                    xhr2.send("nombre='Pasajero seguro'&id_usuario=" + <?php echo $id_usuario; ?> + "&puntos=" + puntaje);
                  } else {
            ...
            xhr.send("id_juego=" + <?php echo $id_juego; ?> + "&puntaje="+puntaje + "&id_modulos=" + <?php echo $id_modulos; ?> + "&nombre_leccion='Transporte escolar'"+ "&puntajeInicial=" + <?php echo $puntajeInicial; ?>);
