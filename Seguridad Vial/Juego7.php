...
<body>
 ...
 <!-- MENU -->
    <?php 
       ...
       $sql = "SELECT * FROM modulos WHERE nombre='Reglas basicas' AND usuarios_id = $id_usuario"; //aqu� var�a el nombre del m�dulo
       $res = $con->query($sql);
       ...
       $sql = "SELECT * FROM lecciones WHERE nombre='Cruce seguro en las calles' AND desbloqueado = 1 AND modulos_id = $id_modulos"; //aqu� var�a el nombre del m�dulo
       $res = $con->query($sql);
       ...
       // Verifico si existe ya un registro con esos datos
        $sql = "SELECT * FROM juegos WHERE nombre='Cruce seguro en las calles' AND tipo='Puzzle'  AND lecciones_id = $id_lecciones AND lecciones_modulos_id = $id_modulos";
        $res = $con->query($sql);
        ...
        if($fila == 0){
        $sql = "INSERT INTO juegos (nombre, tipo, desbloqueado, puntaje, lecciones_id, lecciones_modulos_id) VALUES ('Cruce seguro en las calles','Puzzle',0,0,$id_lecciones ,$id_modulos);";
        $res = $con->query($sql);
        }
    ...
    tiempoAtras = setInterval(() => {
        ...
        if (timer == 0) {
            ...
            Swal.fire({
                ...
                cancelButtonText: '<a class="quitar" href="Juego7.php">Volver a jugar</a>',
...
function bloquearTarjetas() {
    for (let i = 0; i <= 24; i++) {
        ...
        tarjetaBloqueada.innerHTML = `<img src="../imagenes/memoramas/Leccion3/${piezas[i]}.jpg" alt="">`;
...
function seleccionar(casilla) {
    ...
    if (num_click == 1) {
        ...
        if (fin == true) {
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4) {
                  if (xhr.status === 200) {
                    ...
                    xhr2.send("nombre='Cruzando con seguridad'&id_usuario=" + <?php echo $id_usuario; ?> + "&puntos=" + puntos);
                  } else {
            ...
            xhr.send("id_juego=" + <?php echo $id_juego; ?> + "&puntaje="+puntos + "&id_modulos=" + <?php echo $id_modulos; ?> + "&nombre_leccion='Comportamiento peatonal seguro'");
            ...
            Swal.fire({
                ...
                confirmButtonText: '<a class="quitar" href="leccion3-2.php">Siguiente</a>'
            })
            //-------
...
function CambiarPiezas() {
    ...
    for (var casilla = 0; casilla <= 24; casilla++) {
        ...
        document.getElementById(casilla).src = "../imagenes/memoramas/Leccion3/" + imagen + ".jpg";
    }
}
